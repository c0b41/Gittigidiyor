...
use SM\Gittigidiyor\GittigidiyorException;
use SoapClient;
use SOAPHeader;
...
class GittiGidiyor {
  ...
  private function checkConfig($conf){
    ...
    if(!isset($conf['apikey'])  ||
      !isset($conf['secret_key']) ||
      !isset($conf['nick']) ||
      !isset($conf['password']) ||
      !isset($conf['auth_user']) ||
      !isset($conf['auth_password']) ||
      !isset($conf['lang'])) {

      throw new GittigidiyorException("GittiGidiyor Ayarları Girilmedi");
      ...
      $this->secretKey = $conf['secret_key'];
  		...
  		$this->auth_user = $conf['auth_user'];
  		$this->auth_pass = $conf['auth_password'];
  		$this->lang = $conf['lang'];
      return true;
    }
  }
